...
struct LCA{
    ...
    ll get(ll u, ll v){
        ...
        ll index = query(1,0,euler.size()-1,left,right);
        return euler[index];
    }
...
int main() {
    ...
    while(q--){
        ll a,b,c; cin >> a >> b >> c;
        a--; b--; c--;
        ll ab = lca.getdistance(a,b);
        ll bc = lca.getdistance(b,c);
        ll ac = lca.getdistance(a,c);
        ll res = (ab+bc+ac - 2*min({ab,bc,ac}))/2 + 1;
        cout << res << "\n";
    }
}